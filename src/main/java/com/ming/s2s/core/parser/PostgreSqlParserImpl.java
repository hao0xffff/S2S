...
/**
 * PostgreSQL SQL parser strategy implementation
 */
@Component
public class PostgreSqlParserImpl implements SqlParser {

    private static final String SUPPORTED_DB_TYPE = "postgresql";

    @Override
    public boolean supports(String dbType) {
        return SUPPORTED_DB_TYPE.equalsIgnoreCase(dbType) || "postgres".equalsIgnoreCase(dbType);
    }
    ...
    public List<TableMetadata> parse(String sql, String dbType) {
        ...
        // Parse SQL statements using Druid PostgreSQL parser
        ...
        for (SQLStatement statement : statements) {
            if (statement instanceof SQLCreateTableStatement createTableStmt) {
    ...
    private TableMetadata parseTable(SQLCreateTableStatement stmt) {
        TableMetadata table = new TableMetadata();
        
        // Parse table name (PostgreSQL uses double quotes, not backticks)
        // Remove all quotes (both double quotes and backticks) and trim whitespace
        ...
        for (SQLTableElement element : stmt.getTableElementList()) {
            if (element instanceof SQLColumnDefinition colDef) {
                ...
                // Primary key can be defined in column definition (e.g., id SERIAL PRIMARY KEY)
                String colDefStr = colDef.toString().toUpperCase();
                boolean isPrimaryKey = colDefStr.contains("PRIMARY KEY")
                    || colDefStr.contains("SERIAL PRIMARY KEY"); // PostgreSQL SERIAL type
    ...
    private ColumnMetadata parseColumn(SQLColumnDefinition colDef) {
        ...
        // PostgreSQL uses double quotes, not backticks
        ...
        // Handle PostgreSQL specific types (SERIAL, BIGSERIAL, etc.)
        String javaType = convertPostgreSqlType(dbTypeName);
    ...
    /**
     * Extract raw database type with length (e.g., varchar(255), numeric(10,2))
     */
    ...
    /**
     * Convert PostgreSQL specific types to Java types
     */
    private String convertPostgreSqlType(String dbType) {
        if (dbType == null) {
            return "String";
        }
        ...
        if (upperType.equals("NUMERIC") || upperType.equals("DECIMAL")) {
            return "BigDecimal";
        }
        ...
        // Use the standard type converter for common types
        return TypeConverter.convert(dbType);
    }
}
